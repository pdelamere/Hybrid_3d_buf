...

{
  ...
  int err, i, npoint=7, nsamp=1, ncent=0, neval; 
  ...
  err = arms_setup(xinit,ninit,xl,xr,myfunc,mydata,&convex,npoint,dometrop,xprev,&glob_neval,state_ref);
  return err;
}
...
int arms_setup (double *xinit, int ninit, double *xl, double *xr,
                ...
                int *neval, ARMS_STATE **state)

/* Initial setup for derivative-free adaptive rejection sampling with metropolis step */
...
/* *state       : on exit, points to the full state of the initialized arms algorithm */

{
  int err;
  *state = (ARMS_STATE*)malloc(sizeof(ARMS_STATE));
  if(*state == NULL){
    /* insufficient space */
    return 1006;
  }
  ...
  (*state)->lpdf.mydata = mydata;
  (*state)->lpdf.myfunc = myfunc;

  /* set up space required for envelope */
  (*state)->env = (ENVELOPE *)malloc(sizeof(ENVELOPE));
  if((*state)->env == NULL){
    /* insufficient space */
    return 1006;
  }
  ...
  (*state)->metrop = (METROPOLIS *)malloc(sizeof(METROPOLIS));
  if((*state)->metrop == NULL){
    /* insufficient space */
    return 1006;
  }
  (*state)->metrop->on = dometrop;

  /* set up initial envelope */
  err = initial(xinit,ninit,*xl,*xr,npoint,&((*state)->lpdf),(*state)->env,convex,
        neval,(*state)->metrop);
  if(err)return err;
  ...
  if((*state)->metrop->on){
    ...
    (*state)->metrop->xprev = *xprev;
    (*state)->metrop->yprev = perfunc(&((*state)->lpdf),(*state)->env,*xprev);
  }

  //display_env(state->env);
  return 0;
}
...
int arms_sample(double *xsamp, int nsamp, ARMS_STATE *state)
{
  ...
  do {
    /* sample a new point */
    sample (state->env,&state->pwork);

    /* perform rejection (and perhaps metropolis) tests */
    i = test(state->env,&(state->pwork),&(state->lpdf),state->metrop);
    if(i == 1){
      /* point accepted */
      xsamp[msamp++] = state->pwork.x;
    } else if (i != 0) {
...

{
  ...
  //setup
  err = arms_setup(xinit, ninit, xl, xr, myfunc, mydata, convex, npoint,
                   dometrop, xprev, neval, &state);
  if(err)return err;

  //sample
  err = arms_sample(xsamp, nsamp, state);
  if(err)return err;

  /* nsamp points now sampled */
  /* calculate requested envelope centiles */
  for (i=0; i<ncent; i++){
    invert(qcent[i]/100.0,state->env,&(state->pwork));
    xcent[i] = state->pwork.x;
  }
